<script lang="ts">
  ...
  import bgAudio from './assets/7.mp3';
  import fireworkAudio from './assets/firework.mp3';
  import syuAudio from './assets/syu.mp3';
  let canvas: HTMLCanvasElement | null;
  let canvasWidth: number;
  let canvasHeight: number;
  let fireworks: Firework[] = []; // 烟花集合
  let particles: Particle[] = []; // 粒子集合
  // #优化性能
  ...
  function handleCanvasMouseMove(e: MouseEvent) {
    mouseCoord = {
      x: e.pageX - (canvas?.offsetLeft || 0),
      y: e.pageY - (canvas?.offsetTop || 0)
    };
  }
  ...
  // 计算两点之间/起始点到目标点的距离
  //（公式：求两个向量之间距离）
  function calcPointsDistance(sx: number, sy: number, tx: number, ty: number): number {
...
<div class="fireworks-container">
  <h1 class="fireworks-title">用鼠标点击上面的黑色夜空放烟花</h1>
  <audio src={bgAudio} autoplay loop />
  <canvas
    bind:this={canvas}
    width={canvasWidth}
    height={canvasHeight}
...
<style>
  .fireworks-container {
    ...
    background-image: url('./assets/yanhua.jpg');
